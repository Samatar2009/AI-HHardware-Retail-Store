...
async function getDashboardData(locationId: string) {
  ...
  const [inventoryRows, unresolvedAlerts, recentMovements] = await Promise.all([
    ...
    supabase
      ...
      .select('id, alert_type, created_at, product:products(name_en), variant:product_variants(sku)')
      .eq('location_id', locationId)
    ...
    supabase
      ...
      .select('id, movement_type, quantity_change, created_at, product:products(name_en), performed_by_profile:profiles!stock_movements_performed_by_fkey(full_name, phone)')
      .eq('location_id', locationId)
      .order('created_at', { ascending: false })
      .limit(20),
  ])
  ...
  const lowStockCount = rows.filter((r) => r.quantity_on_hand > 0 && r.quantity_on_hand <= r.threshold).length
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <KpiCard icon={DollarSign} label="Total Stock Value" value={formatSLSH(data.totalStockValue)} />
      ...
      <div className="mt-6 grid grid-cols-2 gap-6">
        <div className="overflow-hidden rounded-md border border-stone-200 bg-white">
          ...
          <div className="divide-y divide-stone-100">
            {data.alerts.length === 0 && <p className="p-4 text-sm text-stone-500">No unresolved alerts.</p>}
            {data.alerts.map((alert) => (
              <div key={alert.id} className="flex items-center justify-between px-4 py-3 text-sm">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Link href="/inventory/receive" className="text-xs font-medium text-orange-600 hover:underline">
                    Reorder
                  </Link>
                </div>
              </div>
            ...
            {data.movements.map((m) => (
              <div key={m.id} className="flex items-center justify-between px-4 py-3 text-sm">
                <div>
                  ...
                  <p className="text-xs text-stone-500">{m.performed_by_profile?.full_name || m.performed_by_profile?.phone || 'System'}</p>
                </div>
