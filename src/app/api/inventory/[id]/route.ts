...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  ...
  if (role === 'inventory_manager') {
    ...
    const { data: row } = await supabase.from('inventory').select('location_id').eq('id', params.id).single()
  ...
  const { data, error } = await supabase.from('inventory').update(updates).eq('id', params.id).select().single()
