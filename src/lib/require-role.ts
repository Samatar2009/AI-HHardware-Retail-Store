...
export async function requireRole(allowedRoles: UserRole[]): Promise<RequireRoleResult> {
  ...
  if (!user) {
    return { userId: '', role: 'customer', error: NextResponse.json({ error: 'Not authenticated' }, { status: 401 }) }
  }
  ...
  if (!allowedRoles.includes(role)) {
    return { userId: user.id, role, error: NextResponse.json({ error: 'Forbidden' }, { status: 403 }) }
  }
